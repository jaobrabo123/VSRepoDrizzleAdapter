 ...
 * @property dialect - The SQL dialect to use. Auto-detected from `table`'s own class (`PgTable` -> `"postgresql"`, `CockroachTable` -> `"cockroach"`, `SQLiteTable` -> `"sqlite"`) when omitted; throws `NOT_SUPPORTED` if `table` isn't one of those three and `dialect` wasn't given. An explicit value always overrides detection. Affects placeholder syntax, case-insensitive search behavior, and raw result interpretation.
